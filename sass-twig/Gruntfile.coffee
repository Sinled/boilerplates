  ...
  grunt.initConfig
    ...
    connect:
      ...
      livereload:
        options:
          middleware: (connect) ->
            [lrSnippet, mountFolder(connect, ".tmp"), mountFolder(connect, yeomanConfig.apptempl)]
    ...
    clean:
      dist:
        files: [
          dot: true
          src: [".tmp", "<%= yeoman.templ %>/*", "<%= yeoman.dist %>/*", "!<%= yeoman.dist %>/.git*"]
        ]
    ...
    useminPrepare:
      ...
      html: "<%= yeoman.apptempl %>/index.html"

    usemin:
      options:
        dirs: ["<%= yeoman.dist %>"]

      html: ["<%= yeoman.templ %>/{,*/}*.html"]
